...
export function ClientAdminAddTicketForm() {
  const { data, isLoading } = useSWR('/api/ticket-types/company', fetcher)
    ...
    try {
      ...
      await axios
        ...
        .then((response) => {
          ...
          router.push('/dashboard/empresa/chamados')
          setFiles([])
        })
        .catch((error) => {
          toast.error(
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-md space-y-6">
      ...
      <Button
        ...
        asChild
      >
        <Link href="/dashboard/empresa/chamados">Cancelar</Link>
      </Button>
    </form>
  )
}
