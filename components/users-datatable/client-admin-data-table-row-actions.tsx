...
export function ClientAdminDataTableRowActions<TData>({
  row,
}: DataTableRowActionsProps<TData>) {
  const { mutate } = useSWR('/api/users/company', fetcher)
  ...
  const handleDelete = useCallback(async () => {
    ...
    await axios
      .delete('/api/users/company', { data: { id: usuario.id } })
      .then((response) => {
        setOpenDelete(false)
        toast.success(response.data.message)
        mutate('/api/users/company')
        window.location.href = '/dashboard/empresa/usuarios'
      })
      .catch((error) => {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[180px]">
          <DropdownMenuItem>
            <a href={`/dashboard/empresa/usuarios/${usuario.id}/editar`}>
              Editar
            </a>
          </DropdownMenuItem>
