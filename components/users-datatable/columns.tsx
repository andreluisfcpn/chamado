...
import { roles, statuses } from '@/lib/data'
...
export const columns: ColumnDef<User>[] = [
  {
    ...
    cell: ({ row }) => (
      <div>
        ...
        <div className="text-sm text-zinc-400">{row.original.company.name}</div>
      </div>
    ),
  ...
  },
  {
    ...
    header: 'Chamados Atribuídos',
    cell: ({ row }) =>
      ['ADMINISTRADOR', 'ATENDENTE'].includes(row.original.role) ? (
        <Badge variant="outline" className="rounded-sm font-medium">
          {row.original.assignedTickets.length}
        </Badge>
      ) : (
  ...
  },
  {
    ...
    cell: ({ row }) =>
      ['ADMINISTRADOR_EMPRESA', 'CLIENTE'].includes(row.original.role) ? (
        <Badge variant="outline" className="rounded-sm font-medium">
