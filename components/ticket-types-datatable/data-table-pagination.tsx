...
export function DataTablePagination<TData>({
  ...
  const serverTitle =
    table.getFilteredRowModel().rows.length === 1
      ? 'tipo de chamado encontrado'
      : 'tipos de chamados encontrados'
  return (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-2">
      <div className="flex-1 text-sm text-muted-foreground">
        {table.getFilteredRowModel().rows.length} {serverTitle}.
      </div>

      <div className="flex flex-col md:flex-row items-start md:items-center space-x-0 md:space-x-6 lg:space-x-8 gap-2">
        <div className="flex items-center space-x-2">
          <p className="text-sm font-medium">tipos de chamado por página</p>
          <Select
