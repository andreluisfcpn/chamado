...
import { TicketTypes } from './columns'
...
export function DataTableRowActions<TData>({
  ...
  const tipoChamado = row.original as TicketTypes
  ...
  const handleDelete = useCallback(async () => {
    ...
    await axios
      .delete('/api/ticket-types', { data: { id: tipoChamado.id } })
      .then((response) => {
        setOpenDelete(false)
        toast.success(response.data.message)
        mutate('/api/ticket-types')
        window.location.href = '/dashboard/administrador/chamados/tipos'
      })
      .catch((error) => {
    ...
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[180px]">
          <DropdownMenuItem>
            <a
              href={`/dashboard/administrador/chamados/tipos/${tipoChamado.id}/editar`}
            >
              Editar
            </a>
          </DropdownMenuItem>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              ...
              Você está excluindo um tipo de chamado!
            </AlertDialogTitle>

            <AlertDialogDescription>
              Você está prestes a excluir o tipo de chamado{' '}
